...
$(document).on('ready pjax:scriptcomplete',function(){
    ...
    function runScript() {
        ...
        for(var i = 0; i < mxFormFields.length; i++) {
            ...
            const url = `/upload/question_answers/${mxQuestionId}series.csv`;
            ...
            $.ajax({
                ...
                success: function(data) {
                    $(`#${mxQuestionFieldId}`).autocomplete({
                        minLength: 3,
                        source: processSelectData(data, mxQuestionId)
                    });
                }
            });          
    ...
    function processSelectData(csvData, id) {
        const lines = csvData.split('\n');
        let results = [];
      
        for (let i = 0; i < lines.length; i++) {
          const currentline = lines[i].replace(/^"(.+(?="$))"$/, '$1');      
    ...
    window.onloadTurnstileCallback = function () {
        turnstile.render("#cloudflare-turnstile", {
            ...
            callback: function (token) {
                ...
                document.querySelector('#mx-limesurvey-submit').classList.remove('d-none');
            },
        });
    };
